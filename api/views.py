        ...
        if stat is None:
            ...
            if stat["status"] == "Open":
                ...
                if stat_detail is None:
                    ...
                    return Response(
                        {
                            "status": stat_detail["status"],
                            "time": stat_detail["time"],
                            "msg": stat_detail["msg"],
                        }
                    )
...
def apilist(request, uname, sid):
    ...
    if request.method == "GET":
        ...
        print(request.data)
        try:
            user_db.product_details.insert_one(
                {
                    "shop_id": shop_id,
                    "ptitle": ptitle,
                    "pbrand": pbrand,
...
def apireg(request):
    ...
    if serialized.is_valid():
        ...
        if u is not None:
            return Response({"ok": "User Already Exist"})
        try:
            user_db.client_users.insert_one(
                {
                    "password": password,
                    "address": address,
                    "contact": contact,
...
def apipass(request):
    ...
    if serialized.is_valid():
        ...
        if u is not None and u["password"] == password:
            user_db.client_users.update_one(
                {"contact": contact}, {"$set": {"password": name}}, upsert=True
            )
...
def orders(request, uname):
    if request.method == "GET":
        ...
        for r in user_db.orders.find({"contact": uname}):
            prod_detail = user_db.product_history_record.find_one(
                {"ptitle": r["ptitle"]}
            )
            ...
            product = Product(
                ...
                prod_detail["pmin"],
                prod_detail["pmax"],
                prod_detail["preddisc"],
                prod_detail["pexp"],
                prod_detail["price"],
            )
            products.append(product)
        if not products:
            return Response({"Null": "No orders"})
        serializedList = ProductSerializer(products, many=True)
        return Response(serializedList.data)
...
def orderlist(request, uname):
    ...
    if request.method == "GET":
        ...
        for r in user_db.orders.find({"contact": uname}):
            ...
            product = OrderList(
                r["_id"], r["ord_date"], r["status"], r_nofitems - 1, r["total"]
            )
            products.append(product)
        ...
        try:
            ...
            for each_item, value in orders_items["order"].items():
                if value.get("name") is not None:
                    print("yes")
                    pd = user_db.product_history_record.find_one(
                        {"ptitle": value.get("name")}
                    )
...
def Product_detail(request, uname):
    if request.method == "GET":
        ...
        for r in user_db.product_history_record.find({"ptitle": uname}):
            ...
            products.append(product)
        if not products:
            return Response({"Null": "No products"})
        serializedList = ProductSerializer(products, many=True)
        return Response(serializedList.data)
...
def category(request, uname):
    if request.method == "GET":
        ...
        for r in user_db.product_details.find({"shop_id": uname}).distinct("pcat"):
            ...
            products.append(product)
        if not products:
            return Response({"Null": "No category"})
    ...
    if request.method == "GET":
        ...
        if u is not None:
            ...
            try:
                ...
                return Response({"ok": "Distance not found"})
...
def mail_pdf(request, attachment, contact, mail, oid, tomail):
    ...
    message = (
        "Please find the soft copy of the invoice for order "
        + oid
        + " attached for your reference. Thank you for shopping!\n Regards\nContact Us: "
        + str(contact)
        + " \n "
        + str(mail)
    )
...
def invoice(request, shop_id, ord_id):
    if request.method == "GET":
        ...
        if ord_items is None:
            ...
            attachment = (
                "static/media/invoices/invoice_" + str(ord_items["_id"])[-10:] + ".pdf"
            )
            doc = SimpleDocTemplate(
                attachment, rightMargin=72, leftMargin=72, topMargin=-70
            )
            ...
            name_client = user_db.client_users.find_one(
                {"contact": ord_items["contact"]}
            )
            ...
            story.append(
                Paragraph(
                    "Ordered By : " + str(name_client["name"]).replace("#", " "), h2
                )
            )
            ...
            l = []
            l.append(
                ["Item", "Quantity  ( units )", "Cost / unit ( in € )   ", "Total"]
            )
                ...
                if value.get("name") is not None:
                    ...
                    l.append(
                        [
                            ...
                            value.get("quant"),
                            value.get("price"),
                            float(value.get("price")) * float(value.get("quant")),
                        ]
                    )
                else:
                    print("no")
            ...
            if ord_items["ord_address"] != "not":
                story.append(
                    Paragraph("Delivery Address: " + str(ord_items["ord_address"]), h2)
                )
            else:
